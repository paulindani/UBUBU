function res=multilevel_ubu(niter,burnin,rep,h,gam, grad_lpost, test_function, options)
    %#codegen    
    inputs=struct;
    inputs.gam=gam;
...
function xiarr=gen_xiarr(nbeta,multilevels,h,gam)
    ...
    start_ind(1)=1;
    end_ind(1)=8; %for coupled UBU chains at step size h and h/2, we need 8 gaussians per step h
