function [res,grad_per_ess,bootstrap_grad_per_ess]=poisson_hmc(niter,burnin,rep,numsteps,hrat,bootstrapsamp)
...
beta_data=coder.load("C:\Matlab\Poisson\beta_min.mat");
X=beta_data.X;
...
[R,FLAG,P] = chol(hessian);

% lpostval=lpost(beta_min,pars)
% return;

%beta_init=beta_min*ones(1,rep)+P*(R\randn(nbeta,rep));

tic
res=cell(1,rep);

parfor pit=1:rep
%res{pit}=hmcsampler(niter,burnin,numsteps, partial,h,@(x)(lpost(x,pars)),@(x)(grad_lpost(x,pars)),test_function, beta_init(:,pit));
res{pit}=hmcsampler(niter,burnin,numsteps, partial,h,@(x)(lpost(x,pars)),@(x)(grad_lpost(x,pars)),test_function, beta_min);
end
mean_acc=0;
for pit=1:rep
...
toc
    ...
    function J = lpost(beta,pars)
        J=lprior(beta,pars)+llik(beta,pars);
    end    
...
%         % defense=reshape(beta((numteams*weeks+1):(2*numteams*weeks),1:reps),[numteams,weeks,reps]);
%         % J=J+reshape(reg/2*(sum((attack(:,2:weeks,:)-attack(:,1:(weeks-1),:)).^2,[1, 2])+sum((defense(:,2:weeks,:)-defense(:,1:(weeks-1),:)).^2,[1, 2])),[1,reps]);
%         J=sum((beta).*(hessian_prior*beta),1)/2;
%     end
% 
...
%         gpbeta=grad_lprior(beta);
%         glbeta=grad_llik(beta);
%         grad=gpbeta+glbeta;
%     end
% 
% 
